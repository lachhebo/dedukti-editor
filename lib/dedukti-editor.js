      ...
      processRef.stdout.on('data',
        function(data) { //The callback fonction with the data sent by lptop
          ...
          while((i< array_string_json.length-1) && (stop_cancelled === 0)){ // For each line until we found a error in the check
            ...
            var obj_json = JSON.parse(array_string_json[i]); //I parse each line into a json object
            // !! We need to handle errors sent by the JSon.parse() functions

            if(obj_json[0] === "Feedback") //If it's a feedback message
            {
              if(obj_json[1].event[0] == "Parsed"){ //If it's a parsed confirmation
                array_json_parsed.push(obj_json); //We register the data into a array
              }
              else if (obj_json[1].event[0] === "Cancelled") { //If an error is detected
                array_json_marker.push(obj_json); // We register the object
                ...
                while((j<array_json_parsed.length) && (breaking === 0)){
                  ...
                  */
                  if((array_json_parsed[j][1].span_id == obj_json[1].span_id)){
                    breaking = 1;
                    ...
                    cancelled_where = obj_json[1].span_id;
                  }
                  else{
                    j=j+1;
                  }
              ...
              else if (obj_json[1].event[0] === "Completed") { //Exactely the same thing as with cancelled

                array_json_marker.push(obj_json);
                //console.log("complete entry");
                var breaking = 0;
                var j=0;

                while((j<array_json_parsed.length) && (breaking === 0)){
                  //console.log(array_json_parsed[j][1].span_id, obj_json[1].span_id);
              ...
              else if (obj_json[1].event[0] === "Message") {
                /*
