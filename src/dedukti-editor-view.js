class DeduktiEditorView {
  ...
  setGoals(goallist, editor) {
    ...
    for (i = 0; i < goallist.length; i++) {
      let line = this.createCustomElement("tr", ["goalline"], [], null, this.list_of_proof)
      ...
      */
      ...
      let firstcol  = this.createCustomElement("td", ["goalcolumn"], [],  goallist[i].goal, line)
      let secondcol = this.createCustomElement("td", ["goalcolumn"], [],  null, line)
      let btn       = this.createCustomElement("button", ["btn","btn-xs", "btn-info"], [],  "go !", secondcol)

      //We create a listener for each buttons we add. (TODO : remove them when we clear the goal list)
  ...
  nextFocus() {
    ...
    if (point != null) { //We check a next focus view actually exists.
      editor.setCursorScreenPosition([point.line, point.character]); //We move the cursor
    }
  }

  lastFocus() { //Same as nextFocus
    let editor = atom.workspace.getActiveTextEditor();
    let cursor = editor.getCursorScreenPosition();
    let path = editor.getPath();
